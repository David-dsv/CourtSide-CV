...
def test_firewall_holds_and_recall_lifted():
    """End-to-end on the committed cache, far-wrist OPTED IN: confusion 0/0 AND both
    recall targets (bounce f174, hit f525) recovered. far_wrist_hits is OFF by
    default (it net-regresses live hit precision, see vision/events.py); this test
    runs it ON to lock the dense-pose far-hit path the generator is designed for."""
    data = _load()
    fps, fw, fh, kal, kal_real, wasb, wasb_real, ppf, gt_b, gt_h = data
    sm, b, turns, hits = _pools(fps, fw, fh, kal, kal_real, ppf)
