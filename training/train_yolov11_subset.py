"""
Script d'entraînement YOLOv11 pour les sous-ensembles de datasets
Utilise les datasets créés par create_subset.py
"""
...
class YOLOv11SubsetTrainer:
    """Gestionnaire pour l'entraînement YOLOv11 sur des sous-ensembles"""
    ...
    def __init__(self, base_model='yolo11n.pt', device='auto'):
        ...
        self.datasets_dir = self.training_dir / 'dataset_subset'
        self.runs_dir = self.training_dir / 'runs_subset'
    ...
    def create_dataset_yaml(self, dataset_name, dataset_path, num_classes, class_names):
        ...
        yaml_path = self.training_dir / f'{dataset_name}_subset.yaml'
    ...
    def train_single_model(self, dataset_name, dataset_yaml, epochs=100, imgsz=640,
        ...
        run_name = f"{dataset_name}_subset_{timestamp}"
        ...
        try:
            ...
            final_model_path = self.training_dir / f'{dataset_name}_subset_best.pt'
    ...
    def prepare_datasets(self):
        ...
        raquette_path = self.datasets_dir / 'raquette_subset'
        if raquette_path.exists():
            ...
            logger.warning(f"Dataset raquette subset non trouvé à: {raquette_path}")

        # Configuration pour le dataset tennis ball
        tennis_ball_path = self.datasets_dir / 'tennis ball_subset'
        if tennis_ball_path.exists():
            ...
            logger.warning(f"Dataset tennis ball subset non trouvé à: {tennis_ball_path}")

        # Configuration pour le dataset tennis court
        tennis_court_path = self.datasets_dir / 'tennis_court_id_yolo_subset'
        if tennis_court_path.exists():
            ...
            logger.warning(f"Dataset tennis court subset non trouvé à: {tennis_court_path}")
    ...
    def train_all_models(self, epochs=100, batch_size=16):
        ...
        logger.info("\n" + "="*60)
        logger.info("ENTRAÎNEMENT YOLOV11 SUR SOUS-ENSEMBLES")
        logger.info("="*60)
        ...
        if not datasets:
            ...
            logger.info("\nVeuillez d'abord créer les sous-ensembles avec:")
            logger.info("  python training/create_subset.py")
            return
...
def main():
    parser = argparse.ArgumentParser(description='Entraînement YOLOv11 sur sous-ensembles')
    ...
    # Créer le trainer
    trainer = YOLOv11SubsetTrainer(base_model=args.model, device=args.device)
