...
def main():
    ...
    for fidx, (gx, gy) in gtbf.items():
        ...
        for i, b in enumerate(pboxes):
            ...
            if asp < 1.1:
                continue
            rank = (h * float(pconfs[i]) if y2 >= net_y
                    else _far_score(xr, asp, float(pconfs[i])))
            cand.append((rank, i))
        ...
        for pi in far_pool:
            ...
            s = _far_score(cx / fw, asp, float(pconfs[pi])) + SKEL * min(1.0, nkp / 10.0)
            scored.append((s, pi, nkp, cx, (box[1] + box[3]) / 2))
