...
- **Prochain jalon :** généralisation de l'**homographie** aux angles obliques/amateurs — le dernier verrou. Le tracker balle WASB (`--ball-tracker wasb`) + détecteur de rebond robuste atteignent F1 0.83 sur felix (et WASB marche même sur l'angle oblique de felix). Le détecteur de court keypoints, lui, décroche en oblique (felix : 3/14 ; broadcast : 14/14) → homographie en fallback scalaire. Lever = détecteur de court angle-robuste (fine-tune GPU, ou méthode amateur arXiv 2404.06977). Puis finir le moteur propre vidéo→JSON.
- **Bloqueurs :** pas de bloqueur sur broadcast. Pour l'homographie en angle amateur : pas de GPU local pour fine-tuner le détecteur de court (Mac/MPS).
- **Dernière décision prise :** 2026-06-19 — WASB heatmap = **GO** (F1 0.30→0.83 end-to-end ; le « NO-GO » initial venait d'un bug de prétraitement chez moi — il FAUT la normalisation ImageNet). Tracker par défaut = `kalman` (pas de download requis), `--ball-tracker wasb` pour le meilleur F1. Homographie keypoints OK en broadcast, fallback scalaire flaggé en oblique.
