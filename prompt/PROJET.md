...
- **Palier actif :** 1 (CLI local), front web (`web/`) prêt à brancher (mock/fixtures réels).
- **En cours :** événements quasi parfaits sur le clip de référence (2026-07-02, `feat/accuracy-overhaul`) : rappel **17/17** (9/9 rebonds + 8/8 frappes) vs la GT humaine, **confusion 0/0, zéro marqueur fantôme**, service détecté en bonus — mesuré LIVE ×3 (voir `docs/research/accuracy-overhaul-CR.md`). Le hero tourne en `--homography` (14/14 keypoints, rms 3.5 px) → vitesses/profondeur métriques ; `_stats.json` émet désormais `homography_H`. Web : l'offre post-analyse affiche enfin fatigue réelle, issues de rallyes agrégées, comparaison CD/RV, side-swaps, insights coach ancrés frame.
- **Prochain jalon :** généralisation aux angles obliques/amateurs (le verrou homographie AUTOMATIQUE demeure : felix 3/14 keypoints → semi-auto calibration OK, auto = pilote GPU/RF-DETR). Étendre la GT à un 2e clip (anti-overfit) + brancher le front sur un vrai backend (palier 2).
- **Bloqueurs :** pas de bloqueur sur broadcast. Oblique auto : pas de GPU local pour fine-tuner le détecteur de court.
- **Dernière décision prise :** 2026-07-02 — la GT n'annote ni le service ni le rituel pré-service : les 2 marqueurs « extra » du hero sont RÉELS (vérifiés à l'image) et la GT reste inchangée (benchmark humain intouchable). Les caches d'événements sont reconstruits depuis des dumps live (`scripts/rebuild_demo3_event_cache_from_dump.py`) chaque fois que les poses/le tracking évoluent — un cache gelé ment.
